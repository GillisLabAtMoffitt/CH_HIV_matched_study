...
# Logistic regression model: CH outcome, model to include HIV, age, race, sex, and smoking status only.
```{r}
glm(CH_status ~
      hiv_status + age_at_diagnosis_corrected + 
      new_race + sex + DERIVED_TOBACCO_SMOKING_STATUS_DESC,
    family = "binomial",
    data = CH_HIV_data) %>%
  tbl_regression(exponentiate = TRUE,
                 intercept = TRUE) %>%
...
# Logistic regression model: CH outcome, model to include HIV, age, race, sex, smoking status, horvath13.
```{r}
glm(CH_status ~
      hiv_status + age_at_diagnosis_corrected + 
...
# Logistic regression model: CH outcome, model to include HIV, age, race, sex, smoking status, pheno_age_accel.
```{r}
glm(CH_status ~
      hiv_status + age_at_diagnosis_corrected + 
      new_race + sex + DERIVED_TOBACCO_SMOKING_STATUS_DESC + pheno_age_accel,
    family = "binomial",
    data = CH_HIV_data) %>%
  tbl_regression(exponentiate = TRUE,
                 intercept = TRUE) %>%
...
# Logistic regression model: CH outcome, model to include HIV, age, race, sex, smoking status, dunedin_pace.
```{r}
glm(CH_status ~
      hiv_status + age_at_diagnosis_corrected + 
